...
// bước 1 : khai báo biến và lấy giá trị từ input

// bước 2: xử lý tính lương
document.getElementById("button-convert").onclick = function () {
  let workingDaysInput = document.getElementById("workingDays").value;
  let basicSalaryInput = document.getElementById("basicSalary").value;
  let workingDays = Number(workingDaysInput);
  let basicSalary = Number(basicSalaryInput);

  let totalSalary = workingDays * basicSalary;

  // bước 3 hiển thị
  document.getElementById(
    "result-salary"
  ).innerHTML = `Tổng lương nhân viên: ${totalSalary} VND`;
};
